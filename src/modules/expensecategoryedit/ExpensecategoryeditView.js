...
import { SET_PAGE_REFERSH,SET_RIGHT_ICON_SHOW } from '../AppState';
 ...
 class ExpenseCategoryeditScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  async  updateExpenseCategoryApiCall() {
        try {
          console.log(this.state);
           ...
           let response = await fetch(
            CONSTANTS.UPDATE_EXPENSES_CATEGORY_API+'/'+this.state.auth_token,
            { 
              headers: {
  ...
  async updateExpenseCategory(){
    try {
           ...
           var user=  await this.updateExpenseCategoryApiCall();
    ...
    justifyContent: 'space-around',borderBottomWidth:1,borderColor:colors.primaryLight}} >
           <View style={{flax:1,width: '90%',padding:15}} >
                 ...
                 <TouchableOpacity disabled={this.state.isDisabled}  onPress={() => this.updateExpenseCategory()} >
                       <Image
                        resizeMode="contain"
                        source={saveIcon}
                       />
       ...
       <View style={{paddingTop:30}}>
            ...
            <View style={{padding:15,paddingTop:5}}>
                <TouchableOpacity disabled={this.state.isDisabled}  onPress={() => this.updateExpenseCategory()}  style={{justifyContent:"center",alignItems:'center',backgroundColor:colors.primaryLight,borderRadius:2,padding:10}}>
...
export default connect(mapStateToProps, mapDisptachToProps)(ExpenseCategoryeditScreen)
